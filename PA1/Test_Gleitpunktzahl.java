 ...
 */
	...
	public static void test_Gleitpunktzahl() {
		...
		/*Normalisierung*/
		System.out.println("1. Test der Normalisierung Gleitpunktzahl");
		try {
			// Test: Normalisierte bleibt normalisiert
			System.out.println("Test: normalisiert bleibt");
			...
			if (x.compareAbsTo(gleitref) != 0
					|| x.vorzeichen != gleitref.vorzeichen) {
				System.out.println("Die Zahl wurde unnötig normalisiert und ist jetzt " + x.toString());
				printErg(x.toString(), gleitref.toString());
			} else {
				System.out.println("    Richtiges Ergebnis\n");
			}
